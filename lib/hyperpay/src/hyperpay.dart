...
import 'package:musaneda/hyperpay/src/extensions/payment_mode_ext.dart';
import 'package:musaneda/hyperpay/src/extensions/payment_status_ext.dart';
...
class HyperpayPlugin {
  ...
  Future<String> get getCheckoutID async {
    try {
      ...
      final Map resBody = json.decode(response.body);

      if (resBody['result'] != null && resBody['result']['code'] != null) {
        switch (resBody['result']['code']) {
          case '000.200.100':
            _checkoutID = resBody['id'];
            break;
          case '200.300.404':
            throw HyperpayException(
              resBody['result']['description'],
              resBody['result']['code'],
              resBody.containsKey('parameterErrors')
                  ? resBody['result']['parameterErrors']
                      .map(
                        (param) =>
  ...
  Future<Map<String, dynamic>> paymentStatus(String checkoutID,
    ...
    try {
      ...
      if (resBody['result'] != null && resBody['result']['code'] != null) {
        log(
          '${resBody['result']['code']}: ${resBody['result']['description']}',
